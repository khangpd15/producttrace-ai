...
async function ensureEmbeddingTopology() {
  ...
  try {
    ...
    await channel.assertQueue(
      RABBITMQ.DLQ_ROUTING_KEYS.EMBEDDING_REINDEX,
      {
        durable: true,
      },
    );
    ...
    console.log(
      `Bound ${RABBITMQ.QUEUES.EMBEDDING_REINDEX} ← [${RABBITMQ.EXCHANGE}] ${RABBITMQ.ROUTING_KEYS.EMBEDDING_REINDEX_REQUESTED}`,
    );
...
async function bootstrap() {
  ...
  console.log(
    `Nest AI Service running on ${port}`,
  );
